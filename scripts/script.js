...
for (i = 1; i < NBprojet + 1; i++) {
  ...
  urlImg.style.background = `url(./assets/imgs/${
    textTable[NBprojet - i].imgName
  }) no-repeat 50% 50%`;
...
function cycle() {
  ...
  for (i = 1; i < NBprojet + 1; i++) {
    ...
    // console.log(change);
    img.style.background = `url(./assets/imgs/${
      textTable[change - 1].imgName
    }) no-repeat 50% 50%`;
...
function dotRemplissage() {
  ...
  bgImage.style.background = `url(./assets/imgs/${
    textTable[dotNumber - 1].imgName
